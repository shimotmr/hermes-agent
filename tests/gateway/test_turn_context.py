...
class TestTurnRunner:
    ...
    def test_empty_response_preserves_agent_persistence_contract(self):
        """The empty-response normalization branch must match the normal branch."""

        class _EmptyPersistedAgent:
            def __init__(self, **kwargs):
                self.model = kwargs["model"]
                self.session_id = kwargs["session_id"]
                self.tools = []
                ...
                self.session_prompt_tokens = 0
                self.session_completion_tokens = 0
                self._last_session_persisted = False

            def run_conversation(self, _message, **_kwargs):
                self._last_session_persisted = True
                return {
                    "final_response": "",
                    "completed": True,
                    "messages": [],
        ...
        assert result["agent_persistence_owned"] is True
