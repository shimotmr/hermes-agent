...
_SCHEMA_EVIDENCE = "hermes.update.evidence-event.v1"
_SHA_LEN = 40
...
def apply_overlap_to_matrix(
    ...
    """Invalidate only criteria whose declared ownership intersects drift."""
    result = json.loads(json.dumps(matrix))
    criteria = result.get("criteria")
    if not isinstance(criteria, list):
        raise ValueError("acceptance-matrix-criteria-empty")
    for criterion in criteria:
        if report.classification == "indeterminate" and criterion.get("required") is True:
            criterion["status"] = "invalidated"
            criterion["invalidation_reason"] = "overlap-indeterminate"
            continue
        if report.classification != "overlap":
            continue
        rules = criterion.get("path_rules") or []
        if any(
            ...
            criterion["status"] = "invalidated"
            criterion["invalidation_reason"] = "path-overlap"
    result["overlap"] = asdict(report)
    return result


def _validate_event_shape(event: object, expected_sequence: int, previous_hash: str | None) -> dict[str, Any]:
...
def _read_evidence_bytes(raw: bytes) -> list[dict[str, Any]]:
    if not raw:
        return []
    ...
    for line_number, line in enumerate(raw.splitlines(), start=1):
        ...
        event = _validate_event_shape(parsed, line_number, previous_hash)
        events.append(event)
...
def validate_evidence(path: Path) -> list[dict[str, Any]]:
    return _read_evidence_bytes(Path(path).read_bytes())
    ...
    *,
    ...
    try:
        ...
        with os.fdopen(descriptor, "r+b", closefd=False) as handle:
            ...
            handle.seek(0)
            events = _read_evidence_bytes(handle.read())
            event: dict[str, Any] = {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "classify":
        ...
        return 1 if report.classification == "indeterminate" else 0
    if args.command == "append-evidence":
        ...
        return 0
    _print_json(validate_evidence(args.path))
    return 0
