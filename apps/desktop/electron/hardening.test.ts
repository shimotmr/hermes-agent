...
test('local connection metadata does not decrypt a dormant remote token', () => {
  const source = readMain()
  const fnStart = source.indexOf('async function sanitizeDesktopConnectionConfig(')
  assert.notEqual(fnStart, -1, 'sanitizeDesktopConnectionConfig must exist in main.ts')
  const fnEnd = source.indexOf('\nfunction ', fnStart + 1)
  ...
  assert.match(
    ...
    'local mode must not synchronously enter the OS credential store for an unused remote token'
  )
  assert.match(
    body,
    /const storedRemoteTokenSet = Boolean\(block\.token && typeof block\.token === 'object' && block\.token\.value\)/,
    'the renderer can derive the set flag from the opaque stored envelope'
  )
  assert.match(
    body,
    /remoteTokenSet: mode === 'local' \? storedRemoteTokenSet : Boolean\(remoteToken\)/,
    'remote routes still validate decrypted material while local mode preserves metadata'
  )
})

test('registry metadata never decrypts stored connection tokens', () => {
  const source = readMain()
  const fnStart = source.indexOf('function sanitizeRegistryConnection(')
  assert.notEqual(fnStart, -1, 'sanitizeRegistryConnection must exist in main.ts')
  const fnEnd = source.indexOf('\nfunction ', fnStart + 1)
  const body = source.slice(fnStart, fnEnd === -1 ? undefined : fnEnd)

  assert.doesNotMatch(body, /decryptDesktopSecret\(/, 'registry enumeration must not synchronously enter the OS credential store')
  assert.match(
    body,
    /const storedTokenSet = Boolean\(token && typeof token === 'object' && token\.value\)/,
    'the renderer only needs the opaque envelope set flag'
  )
  assert.match(body, /tokenSet: storedTokenSet/, 'the opaque set flag must reach the renderer')
  assert.match(body, /tokenPreview: ''/, 'registry metadata must not expose a decrypted token preview')
})
